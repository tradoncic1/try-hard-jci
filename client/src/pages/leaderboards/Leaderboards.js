        ...
        <Col md={8}>
          {isLoading ? (
            ...
            <div className="Leaderboards-Wrap">
              ...
              {!first ? null : (
                <Card
                  className="Leaderboards-First"
                  onClick={() =>
                    props.history.push(`/profile/${first.username}`)
                  }
                >
                  <Row>
                    {first.avatar ? (
                      <img
                        className="Leaderboards-FirstImg"
                        src={first.avatar}
                      />
                    ) : (
              ...
              {!second ? null : (
                <Card
                  className="Leaderboards-Second Leaderboards-Row"
                  onClick={() =>
                    props.history.push(`/profile/${second.username}`)
                  }
                >
                  <div>
                    2.{" "}
                  ...
                  <div>{second.exp}XP</div>
                </Card>
              )}
              {!third ? null : (
                <Card
                  className="Leaderboards-Third Leaderboards-Row"
                  onClick={() =>
                    props.history.push(`/profile/${third.username}`)
                  }
                >
                  <div>
                    3.{" "}
